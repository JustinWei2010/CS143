...
Part D:
We split up the last part of the lab by dividing the select function and working together on the load function. We chose to implement the select function by pre-evaluating the range of keys that was to be read from the tree. The range would be dependent on the conditions given by the WHERE clause in the select statement. The range represents the minimal amount of keys that has to be read from the tree to work. Besides the specifications given to us we also do not read the values in from table when select calls for count(*), it only increments the counter variable and outputs it out in the end. This is a slight optimization to the speed of select queries. 